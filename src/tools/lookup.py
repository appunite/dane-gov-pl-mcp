...
async def search_datasets_advanced(
        titile_filter: TitleFilter, 
        keyword_filter: Optional[KeywordFilter], 
        query_filter: Optional[QueryFilter], 
        other_filters: Optional[OtherFilters]) -> dict:
    """Advanced dataset search with nested filters."""
    set_title_filters = title_filter.model_fields_set
    if set_title_filters:
